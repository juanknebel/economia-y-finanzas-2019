...
linea_muerte <- function( pmes_cero )
{
  ...
  prediccion_final  <-  cbind(  dataset[ foto_mes==201906, c("numero_de_cliente","foto_mes", "clase_ternaria") ], aplicacion_prediccion )
  
  #le doy nombre a las columnas
  colnames( prediccion_final )  <-  c( "numero_de_cliente", "foto_mes", "clase01", "prob_positivo" )
  ...
  ganancia = paste(pred,":",sum(  prediccion_final[ prob_positivo>0.025,   ifelse( clase01 == 1  , 19500, -500) ] ))
  print(ganancia)
  #Genero las CUATRO salidas
  #grabo todas las probabilidad, simplemente para tenerlo

  #grabo ganancia final
fwrite( as.data.table(ganancia), 
        file=paste( "ganancia_xgb_", pred, ".txt", sep="" ), 
        col.names=FALSE, 
        sep="\t", 
        eol = "\r\n")
  ...
  fwrite( prediccion_final[ order( -prob_positivo) ],
          file= paste( "ldm_prob_xgb_", pred, ".txt", sep="" ),
          sep="\t",
          eol = "\r\n")

  #grabo la importancia de las variables
  write.table(  xgb.importance( model = modelo ),
                file=paste( "ldm_importancia_xgb_", pred, ".txt", sep="" ),
                sep="\t",
                eol = "\r\n"
  )
...
fwrite( as.data.table( prediccion_final[ prob_positivo > 0.025  , "numero_de_cliente" ] ),  
        file=paste( "ldm_entregar_xgb_", pred, ".txt", sep="" ),
        col.names=FALSE, 
        sep="\t", 
        eol = "\r\n")
...
dataset   <-   fread( "paquete_premium_exthist_sinAVG.txt.gz" )
