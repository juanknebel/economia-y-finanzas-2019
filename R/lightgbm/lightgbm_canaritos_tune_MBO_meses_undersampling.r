...
env$experimento          <-  4303
env$undersampling        <-     0.10
env$cant_canaritos       <-     0
env$num_canaritos        <-     0
...
lgb$num_leaves              <-   1024
lgb$max_depth               <-     -1    # arbol creced libre
lgb$min_data_in_leaf        <-      1    # arbol creced libre
lgb$min_gain_to_split       <-      0    # arbol creced libre
lgb$min_sum_hessian_in_leaf <-      0.0  # arbol creced libre
lgb$feature_fraction        <-    1.0    # arbol creced libre ?
lgb$lambda_l1               <-    0.0    # arbol creced libre ?
lgb$lambda_l2               <-    0.0    # arbol creced libre ?
env$lightgbm <- lgb
...
modelo_lightgbm_futuro <- function( ptrain, ptest, ptest_clase, pclase_nomcampo, pclase_valor_positivo, pproblema, 
                                    plearning_rate, pnum_iterations )
{
...
modelo_lightgbm_ganancia_MBO_directo <- function( x )
{
  ...
  st_parametros = paste("ventana=", x$pventana,                   ", ",
                        ...
                        "lambda_l1=",        env$lightgbm$lambda_l1,            ", ",
                        "lambda_l2=",        env$lightgbm$lambda_l2,           ", ",
                        "min_gain_to_split=",env$lightgbm$min_gain_to_split,            ", ",
                        "min_data_in_leaf=", env$lightgbm$min_data_in_leaf, ", ",
                        "max_depth=",        env$lightgbm$max_depth,        ", ",
                        "feature_fraction=", env$lightgbm$feature_fraction, ", ",
...
agregar_canaritos( dataset_grande, env$cant_canaritos )
