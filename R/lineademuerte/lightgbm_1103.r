...
dead_line = function(month_to_process) {
  ...
  # Experimento 17105, parametros que mejoraron la aplicacion del 201904
  # ventana=10, probcorte=0.269931907750836, num_iterations=39, learning_rate=0.162654548685709,
  # lambda_l1=18.6006107425395, lambda_l2=35.3730934046418, min_gain_to_split=0.646206255202216,
  # min_data_in_leaf=87, max_depth=6, feature_fraction=0.653286567813752, max_bin=255, subsample=1
  ...
  if (predict_month != competition_month) {
    ...
    if (the_profit < 0) {
      stop("Ganancia negativa, not tiene sentido seguir, aborto")
    }
  }
}
...
dataset <- fread( "paquete_premium_exthist_sinAVG_lm_original.txt.gz" )
file_name = "lightgbm_dead_line_1103"
from = 201806
to = 201904
competition_month = 201906
# La probabilidad default de corte
prob_class_down = 0.0272
