...
#----------------------
#Los canaritos, pero SIN crearlos
#Simplemente permutando la clase

num_canaritos <- 169*20

clase_original <-  dataset$clase01

dataset_sub <-  dataset[ (Visa_mfinanciacion_limite<167.9 | is.na(Visa_mfinanciacion_limite<167.9) ) & mcuentas_saldo< -8228.3 & marketing_coss_selling>=6, ]

dataset_sub <-  dataset_sub[ , c("clase01")]

total <- nrow( dataset_sub )
pos_total   <- sum(dataset_sub$clase01)
neg_total   <- total - pos

hull_neg <-  rep( 0, neg_total+1 )
hull_pos <-  rep( 1e20, pos_total+1 )

titulo <-  paste0( "ROC Curve  pos=", pos,  "   neg=", neg, "  canaritos=", num_canaritos) 
graficar_init( dataset_sub , "ROC_canaritos_5000", titulo,  archivo=TRUE )

vclase <-  dataset_sub$clase

for( i in 1:num_canaritos )
{
  clase_randomizada  <- sample( dataset_sub$clase01, replace=FALSE )
  
  #acumulo positivos y negativos,  operacion vectorial
  neg_acum  <- cumsum( 1- clase_randomizada )
  pos_acum  <- cumsum( clase_randomizada )
  
  idx <-  pos_acum/( pos_acum + neg_acum )
  
  umbral <- pos_total/(pos_total+neg_total)
  pos2_acum <-  ifelse( idx < umbral , pos_total - pos_acum, pos_acum )
  neg2_acum <-  ifelse( idx < umbral , neg_total - neg_acum, neg_acum )
  
  tbl <- as.data.table( cbind( neg2_acum, pos2_acum ) )
  
  tbl_neg <-  tbl[   ,  list( "pos2_acum" = max( pos2_acum ) ),  by=neg2_acum  ]
  hull_neg[ tbl_neg$neg2_acum+1 ] <- pmax( hull_neg[ tbl_neg$neg2_acum+1 ] , tbl_neg$pos2_acum  )

  tbl_pos <-  tbl[   ,  list( "neg2_acum" = min( neg2_acum ) ),  by=pos2_acum  ]
  hull_pos[ tbl_pos$pos2_acum+1 ] <- pmin( hull_pos[ tbl_pos$pos2_acum+1 ] , tbl_pos$neg2_acum  )
}

points( 0:neg_total,    hull_neg, type="p", col="red", pch=20 )
points(    hull_pos, 0:pos_total, type="p", col="red", pch=20 )
