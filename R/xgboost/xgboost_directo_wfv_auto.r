#En construccion
#Advertencia : puede causar una explosion
quit(save="no")
...
env$experimento          <-  22666
env$procesar_solo_201904 <-  FALSE

env$undersampling        <-  1.0
env$unir_bajas           <-  FALSE     #poner en TRUE  si se quiere entrenar con  positivos={BAJA+2, BAJA+1}
...
data$clase_test           <-  c("BAJA+2")
data$clase_train          <-  c("BAJA+2","BAJA+1")
...
mbo <- list()
mbo$iteraciones       <-   10   # cuantas iteraciones hace la Optimizacion Bayesiana
...
hiper$programa       <-  "xgboost_directo_wfv_auto.r"
hiper$algoritmo      <-  "xgboost"
hiper$busqueda       <-  "MBO"
hiper$estimacion     <-  "mes_futuro"
hiper$observaciones  <-  ifelse( env$unir_bajas, "BAJA+2,BAJA+1", "BAJA+2")
hiper$separador      <-  "\t"
...
Gxgb_actual <- list( "ganancia_mejor"=0, "prob_corte"=0 )
...
fganancia_logistic_xgboost_ACTUAL   <- function(probs, clases) 
{
   ...
   if( gan > Gxgb_actual$ganancia_mejor )
   {
     #setwd(env$directory$work)
     #fwrite( tbl, file="proba.txt", sep="\t" )
   
     Gxgb_actual$prob_corte       <<- vprob_corte
     Gxgb_actual$ganancia_mejor   <<- gan
   }
...
modelo_xgboost_actual <- function( ptrain, ptest, xx )
{
  ...
  if( ganancia >=  glob_ganancia_mejor )
  {
    ...
    glob_prob_corte_mejor <<- Gxgb_actual$prob_corte
  ...
  return( list("ganancia_test"= ganancia, 
               ...
               "prob_corte"= Gxgb_actual$prob_corte,
...
modelo_xgboost_futuro <- function( ptrain, ptest, zz, pprobcorte )
{
...
modelo_xgboost_ganancia_MBO_directo <- function( x )
{
  ...
  dataset_grande[  (sample < env$undersampling | clase01_train== 1) & mes<=(vmes_actual_train+x$pventana-1)  &  mes>=vmes_actual_train & mes!=vmes_actual_test ,
                   train:=1 ]

  dactual_train <-   xgb.DMatrix( data  = data.matrix( dataset_grande[ train== 1, 
                                                                       !c("sample","train", "clase01_train", "clase01_test", env$data$campo_id, env$data$clase_nomcampo), 
                                                                       with=FALSE]),
...
optimizacion_bayesiana <- function( pmes_cero )
{
  ...
  vpesos <- ifelse( vazar<0.5, 1.0, 1.0+ epsilon )
  
  dactual_test  <<-   xgb.DMatrix( data= data.matrix(dataset_grande[mes==vmes_actual_test, !c("sample","train","clase01_train","clase01_test",env$data$campo_id, env$data$clase_nomcampo), with=FALSE]),
                                   label= dataset_grande[ mes==vmes_actual_test, clase01_test],
                                   weight = vpesos,
                                   missing= NA
                                 )
  
  
  dfuturo_test  <<-   xgb.DMatrix( data= data.matrix(dataset_grande[mes==vmes_futuro_test, !c("sample","train","clase01_train","clase01_test",env$data$campo_id, env$data$clase_nomcampo), with=FALSE]),
                                   label= dataset_grande[ mes==vmes_futuro_test, clase01_test],
                                   missing= NA
                                 )
  ...
  if(  vmes_futuro >= 1 )
  {
    ...
    dtrain_final  <-   xgb.DMatrix( data= data.matrix(dataset_grande[ train==1 , !c("sample","clase01_train","clase01_test","train", env$data$campo_id, env$data$clase_nomcampo), with=FALSE]),
                                    label= dataset_grande[ train==1 , clase01_train ], 
                                    missing= NA 
                                  )
    ...
    testing_prediccion  <- predict( mfinal,  
                                    as.matrix(test_final[,!c("sample","clase01_train","clase01_test","train", env$data$campo_id, env$data$clase_nomcampo), with=FALSE ] ) )
...
dataset_grande[, clase01_test  := as.numeric( get(env$data$clase_nomcampo) %in% env$data$clase_test  )] 
dataset_grande[, clase01_train := as.numeric( get(env$data$clase_nomcampo) %in% env$data$clase_test  )] 

if( env$unir_bajas )  dataset_grande[, clase01_train := as.numeric( get(env$data$clase_nomcampo) %in% env$data$clase_train  )]
...
dataset_grande[ , train:=0 ]
dataset_grande[ sample<= env$undersampling | clase01_train==1, train:=1 ]
