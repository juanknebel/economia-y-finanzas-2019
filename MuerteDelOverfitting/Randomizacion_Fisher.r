...
graficar_init( dataset_sub , "ROC_randomizar_fischer", titulo,  archivo=FALSE )
...
#Injerto Fisher
#En construccion
#Es tremendamente optimizable, quizas con la libreria Exact ?
...
for( n in 0:NEG )
{
  pos_diagonal <- as.integer( (n/NEG)*POS)
  prob_previa <- 1.0
  
  for( p in  pos_diagonal:POS )
  {
    prob <- bondad_corte_mc( POS, NEG, p, n,FisherVAR)
    if( prob < alfa  & prob_previa > alfa )  
    {
      x <- c( x, n )
      y <- c( y, p )
    }
